...
public class RangedPatrolling : MonoBehaviour
{
    ...
    public bool enableFollowRange;


    void Start()
    {
        attack = GetComponent<AttackGroundRanged>();
    ...
    void Update()
    {
        ...
        if (!knock.knocked && healthEnemy.isAlive)
        {
            ...
            {
                if ((detection.visionArea || healthEnemy.hitted) && player != null && enableFollowRange && GroundDetected())
                {
                    FollowTarget();
                }
                else
    ...
    void SetFollow()
    {
        if (attacking || healthEnemy.hitted)
        {
            enableFollowRange = true;
        }
        if (!detection.visionArea || !GroundDetected())
        {
            enableFollowRange = false;
        }
    }
    ...
    {
        FlipToPlayer();
        if (!healthEnemy.hitted && GroundDetected() && !attacking)
        {
