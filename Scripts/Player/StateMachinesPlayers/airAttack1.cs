...
public class airAttack1 : StateMachineBehaviour
{
    // OnStateEnter is called when a transition starts and the state machine starts to evaluate this state
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        MeleCombat.instance.AttackAir1();
    ...
    {
        if(!MeleCombat.instance.isAttacking || !MeleCombat.instance.HaveHeightToAirAttack())
        {
            if(PlayerDash.instance.airDashed && PlayerDash.instance.CanIAirDash())
            {
                animator.Play("airDash");
            }
            else if(PlayerJump.instance.GoingDown())
            {
                animator.Play("goingDown");
            }
            else
